...
For those who are not familiar with the Rust language environment, we have prepared a [pre-compiled binary for Windows](https://img.atcoder.jp/ahc040/RGoXy7re_windows.zip).
...
- `--T=30` Fix the $T$ value of the generated inputs to the specified one.
- `--sigma=1000` Fix the $\sigma$ value of the generated inputs to the specified one.
...
You can also use a [web visualizer](https://img.atcoder.jp/ahc040/RGoXy7re.html?lang=en) which is more rich in features.
...
Rust言語の環境構築が面倒な方向けに、[Windows用のコンパイル済みバイナリ](https://img.atcoder.jp/ahc040/RGoXy7re_windows.zip)も用意してあります。
...
- `--T=30` 生成される入力の $T$ の値を指定されたものに固定
- `--sigma=1000` 生成される入力の $\sigma$ の値を指定されたものに固定
...
より機能が豊富な[ウェブ版のビジュアライザ](https://img.atcoder.jp/ahc040/RGoXy7re.html?lang=ja)も利用可能です。
